...
- **Server Light sender / receiver**: both phones manually enter the same server IP/domain. The receiver automatically generates a stable device ID from local Android device information and shows a copy button; the sender enters that ID to connect through `server:42110`. No account, manager API, database UI, or manager-web login is required. This is useful for Tailscale/ZeroTier/ngrok or a minimal public VPS.
- **Server Full sender / receiver**: both phones log in with the same account through the manager API. The receiver automatically joins the account with its generated device ID and readable device name, and the sender queries the device list, selects a receiver, then controls it through `control_server`. This mode also starts a small `manager_web` service separately from `manager_api`.
