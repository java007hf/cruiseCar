        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:
                ...
                if path == "/health":
                    ...
                    self._send_json({"ok": True, "data": api.store.list_receivers()})
                    ...
                    self._send_json({"ok": True, "data": api.store.list_senders()})
            ...
            def _authorized(self) -> bool:
                if not api.config.auth_token:
                    return True
                header = self.headers.get("Authorization", "")
                token = header[7:] if header.startswith("Bearer ") else header
                return token == api.config.auth_token
