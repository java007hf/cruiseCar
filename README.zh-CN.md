...
- **服务器 Light 发送端 / 接收端**：两台手机分别手动输入同一个服务器 IP/域名。接收端会根据本机 Android 设备信息自动生成稳定设备 ID；发送端填入该 ID 后通过 `server:42110` 连接控制。不需要账号、manager-api、数据库 UI 或 manager-web 登录。适合 Tailscale/ZeroTier/ngrok 或最小化公网 VPS。
